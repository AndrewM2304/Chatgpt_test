...
export const ManageView = ({
  ...
  const isPasswordReady = passwordHash !== null;

  const handleSharedPasswordSubmit = async (event) => {
    event.preventDefault();
    setSharedMessage("");
    setSharedError("");

    if (!sharedPassword.trim()) {
      setSharedError("Enter a shared passphrase to continue.");
      return;
    }
    if (sharedPassword !== sharedConfirm) {
      setSharedError("Passphrases do not match.");
      return;
    }

    setIsPasswordSaving(true);
    const ok = await onSetAccessPassword(sharedPassword);
    setIsPasswordSaving(false);

    if (ok) {
      setSharedPassword("");
      setSharedConfirm("");
      setSharedMessage("Shared passphrase saved.");
    } else {
      setSharedError("Unable to save the shared passphrase.");
    }
  };
  ...
  return (
    <section className="manage">
      ...
      <div className="settings-card">
        ...
        <p>
          Add an optional shared passphrase to share with your group. You can
          update this at any time.
        </p>
        <form className="settings-form" onSubmit={handleSharedPasswordSubmit}>
          <label htmlFor="shared-passphrase">
            {passwordHash ? "Update shared passphrase" : "Set shared passphrase"}
          </label>
          <input
          ...
          <input
            type="password"
            value={sharedConfirm}
            onChange={(event) => setSharedConfirm(event.target.value)}
          ...
          <button
            className="primary"
            type="submit"
            disabled={!isPasswordReady || isPasswordSaving}
          >
            {passwordHash ? "Update passphrase" : "Save passphrase"}
          </button>
          {(sharedMessage || sharedError) && (
            <p className={sharedError ? "error-text" : "success-text"}>
              {sharedError || sharedMessage}
            </p>
          )}
        </form>
      </div>
